...
class PWMBooleanDevice:
    """
    Generic control class for auxiliary devices such as gear shifting and T-locks, based on boolean input.
    ...
    """
    def __init__(self, device_name_state, controller, true_pulse, false_pulse):
        if controller is None:
        ...
        self.device_name_state = device_name_state
        self.controller = controller
        ...
        self.pulse = false_pulse  # Default to false state
        self.current_state = None

        # Initialize with the false state (e.g., unlocked or low gear)
        self.controller.set_pulse(self.pulse)
        time.sleep(1)
        self.running = True
    ...
    def run_threaded(self, state):
        ...
        if self.current_state != state:
            ...
            # Set the pulse based on the current state (true or false)
            pulse = self.true_pulse if state else self.false_pulse
            self.controller.set_pulse(pulse)

    def run(self, state):
        self.run_threaded(state)
